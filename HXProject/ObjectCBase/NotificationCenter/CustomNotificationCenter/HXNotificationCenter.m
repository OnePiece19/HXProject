...
@interface HXNotificationCenter ()

/** 观察者字典 */
@property (nonatomic, strong) NSMutableDictionary<NSNotificationName, NSPointerArray *> *observerDict;

/** 观察信息字典 */
@property (nonatomic, strong) NSMutableDictionary<NSNotificationName, NSMutableSet<HXObserverInfo *> *> *observerInfoDict;
...
@implementation HXNotificationCenter

+ (instancetype)defaultCenter {
    static HXNotificationCenter *center;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        center = [[HXNotificationCenter alloc] init];
        //注意点：1、通知中心要监听内存警告，接收到内存警告时，可以清除掉观察者为 nil 的对象，回收内存。
...
- (void)addObserver:(id)observer selector:(SEL)aSelector name:(nullable NSNotificationName)aName object:(nullable id)anObject {
    ...
    HXObserverInfo *info = [[HXObserverInfo alloc] initWithObserver:observer selector:aSelector name:aName object:anObject];
...
- (void)addObserverForName:(nullable NSNotificationName)aName observer:(nullable id)observer queue:(nullable NSOperationQueue *)queue usingBlock:(void (^)(HXObserverInfo *info))block {
    if (!aName || !aName.length || !observer) {
        return;
    }
    [self.lock lock];
    ...
    HXObserverInfo *info = [[HXObserverInfo alloc] initWithObserver:observer name:aName queue:queue block:block];
    if (![self hasExistedObserverInfo:info]) {
        NSMutableSet *set = [self.observerInfoDict objectForKey:aName];
        if (!set) {
            set = [NSMutableSet set];
...
- (BOOL)hasExistedObserverInfo:(HXObserverInfo *)newInfo {
    NSMutableSet *set = [self.observerInfoDict objectForKey:newInfo.name];
    if (!set || !set.count) {
        return NO;
    }
    for (HXObserverInfo *info in set) {
 ...
 if (aName != nil) {
     在 observerDict 中 以 aName 为 key 的 NSPointerArray
         if (NSPointerArray 中 有 pointer 和 observer 相同) {
             在 observerInfoDict 中查找 aName 和 observer 都相同的 HXObserverInfo
                 if (HXObserverInfo.object == anObject) {
                     将 HXObserverInfo 从 observerInfoDict 中移除
                 } else {
                     // 因为 HXObserverInfo.object != anObject，所以 observerDict 不能移除 pointer
                 }
                 if (所以的 HXObserverInfo.object == anObject) {
                     将 pointer 从 observerDict 中移除
                 }
         }
 } else if (!aName && anObject) {
     在 observerInfoDict 中遍历 HXObserverInfo
         if (HXObserverInfo.observer == observer && HXObserverInfo.object == anObject) {
             将 HXObserverInfo 从 observerInfoDict 中移除
         }
 } else if (!aName && !anObject) {
     移除 observerDict.NSPointerArray 中，所有 pointer == observer 的 pointer
     移除 observerInfoDict.NSMutableSet<HXObserverInfo *> 中，所有 HXObserverInfo.observer == observer 的 HXObserverInfo.observer
 }
 */
        ...
        if (aName && [key isEqualToString:aName]) { // 通知名一致
            for (NSInteger i = obj.count - 1; i >= 0; i--) {
                ...
                if (observer == pointer) { // 观察者一致
                    ...
                    if (set) {
                        [set enumerateObjectsUsingBlock:^(HXObserverInfo * _Nonnull obj, BOOL * _Nonnull stop) {
            ...
            [self.observerInfoDict enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, NSMutableSet<HXObserverInfo *> * _Nonnull set, BOOL * _Nonnull stop) {
                [set enumerateObjectsUsingBlock:^(HXObserverInfo * _Nonnull obj, BOOL * _Nonnull stop) {
...
- (void)cleanObserverDict {
    ...
    [self.observerDict enumerateKeysAndObjectsUsingBlock:^(NSNotificationName _Nonnull key, NSPointerArray * _Nonnull obj, BOOL * _Nonnull stop) {
        if (obj.count) {
            ...
            [obj compact];        // 删除数组中的nil值
        }
    }];
    [self.lock unlock];
}

/** 删掉观察者信息集合 NSMutableSet 中，观察者信息 HXObserverInfo 的 观察者 为 nil 的对象*/
- (void)cleanObserverInfoDict {
    if (!self.observerInfoDict.count) {
        return;
    }
    [self.lock lock];
    [self.observerInfoDict enumerateKeysAndObjectsUsingBlock:^(NSNotificationName _Nonnull key, NSMutableSet<HXObserverInfo *> * _Nonnull obj, BOOL * _Nonnull stop) {
        [obj enumerateObjectsUsingBlock:^(HXObserverInfo * _Nonnull infoObj, BOOL * _Nonnull stop) {
            if (!infoObj.observer) {
...
- (NSMutableDictionary<NSNotificationName,NSMutableSet<HXObserverInfo *> *> *)observerInfoDict {
